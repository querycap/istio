...
dockerx.build-tools: clean.build-tools clone.build-tools
	cd $(TEMP_ROOT)/tools && wget -q -O - https://patch-diff.githubusercontent.com/raw/istio/tools/pull/1622.patch | git apply -v
	cd $(TEMP_ROOT)/tools/docker/build-tools \
		&& DRY_RUN=1 CONTAINER_BUILDER="buildx build --platform=linux/arm64" ./build-and-push.sh
